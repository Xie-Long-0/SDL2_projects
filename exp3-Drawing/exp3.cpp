...
int Init()
{
    ...
    /* 设置窗口 */
    win = SDL_CreateWindow("加载图片", 100, 100, 400, 300, SDL_WINDOW_SHOWN);
    /* 设置渲染器 */
...
int LoadSrc()
{
    picSur = IMG_Load_RW(SDL_RWFromFile("dots.png", "rb"), 1);
    picSur = SetColorKey(picSur, 0, 0xff, 0xff);
    pic = SDL_CreateTextureFromSurface(renderer, picSur);
    background =
        SDL_CreateTextureFromSurface(renderer, IMG_Load_RW(SDL_RWFromFile("background.png", "rb"), 1));
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        BiltTexture(0, 0, pic, renderer);
        ClipBiltTexture(200,0, pic, clip, renderer);
